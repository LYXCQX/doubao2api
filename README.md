...
内置现代化 Web 管理面板（Vue 3 单文件应用，零构建依赖，国内海外 CDN 智能容灾）。

**访问**：直接在浏览器打开 `http://127.0.0.1:9090` 或 `http://127.0.0.1:9090/admin`（局域网/远程服务器可直接用 `http://<IP>:9090`）。

| 页面 / 模块 | 功能 |
|------|------|
| **状态与登录** | 会话授权状态监控、浏览器模式热切换（静默后台/桌面窗口）、动态服务地址展示与一键复制、豆包 App 免窗口扫码登录、Cookie 一键导入激活、后台实时画面截图 |
| **API 调试测试** | 交互式在线测试（支持极速/思考/专家全模型）、流式打字与单次输出、思维链实时展开、耗时统计、预设提问示例 |
| **系统信息与配置** | 实时监测操作系统平台、Python 版本、服务连续运行时长、监听端口、RPM 频率限制、即用即焚状态及已注册模型清单 |
| **请求日志** | 实时查看最近 100 条请求记录、HTTP 方法、路径、模型、状态码及响应耗时（5s 自动轮询刷新） |

- **多机器无缝适配**：控制台动态根据访问来源生成客户端 Base URL（自动匹配 `window.location.origin`），支持局域网、VPS 及 Docker 远程操作，提供一键复制。
- **智能鉴权**：若开启了 `DOUBAO_API_KEY`，可在页面顶部直接设置/更新密钥，自动保存至本地存储，告别 401 报错困扰。
- **免窗口扫码**：在无图形界面（Linux VPS/Docker）环境下，点击「📱 豆包 App 扫码登录」即可在网页直接扫码完成授权。
